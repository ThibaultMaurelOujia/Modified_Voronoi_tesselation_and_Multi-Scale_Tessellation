...
int main(int argc, char *argv[]) {
    ...
    ARG_CONFIG.BOOL_TENSOR_MERGE = CONFIG.BOOL_TENSOR_MERGE;
    ...
    ARG_CONFIG.BOOL_TENSOR_WRITE = CONFIG.BOOL_TENSOR_WRITE;
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            ARG_CONFIG.velocity_gradient_tensor_components[i][j] = CONFIG.velocity_gradient_tensor_components[i][j];
        }
    }
